...
// function todosGruposFinalizados(){

//   for(let grupo in tabela){

//     const times = Object.values(tabela[grupo]);

//     const completo = times.every(time=>{
//       return (time.v + time.e + time.d) === 3;
//     });
...
//     let ranking = Object.entries(tabela[grupo]);
...
//     classificados[`1º ${grupo}`] = ranking[0][0];
//     classificados[`2º ${grupo}`] = ranking[1][0];

//     terceiros.push({
//       grupo,
//       time: ranking[2][0],
//       pts: ranking[2][1].pts,
//       sg: ranking[2][1].gp - ranking[2][1].gc,
//       gp: ranking[2][1].gp
//     });
//   }
...
//   const melhores = terceiros.slice(0,8);

//   const fase16 = jogosDetalhados.find(
//     b => b.rodada === "16 avos de Final"
//   );
...
//     if(jogo.id===73){
//       jogo.casa = classificados["2º Grupo A"];
//       jogo.fora = classificados["2º Grupo B"];
//     }

//     if(jogo.id===74){
//       jogo.casa = classificados["1º Grupo E"];
//       jogo.fora = melhores[0]?.time || "";
//     }

//     if(jogo.id===75){
//       jogo.casa = classificados["1º Grupo F"];
//       jogo.fora = classificados["2º Grupo C"];
//     }

//     if(jogo.id===76){
//       jogo.casa = classificados["1º Grupo C"];
//       jogo.fora = classificados["2º Grupo F"];
//     }

//     if(jogo.id===77){
//       jogo.casa = classificados["1º Grupo I"];
//       jogo.fora = melhores[1]?.time || "";
//     }

//     if(jogo.id===78){
//       jogo.casa = classificados["2º Grupo E"];
//       jogo.fora = classificados["2º Grupo I"];
//     }

//     if(jogo.id===79){
//       jogo.casa = classificados["1º Grupo A"];
//       jogo.fora = melhores[2]?.time || "";
//     }

//     if(jogo.id===80){
//       jogo.casa = classificados["1º Grupo L"];
//       jogo.fora = melhores[3]?.time || "";
//     }

//     if(jogo.id===81){
//       jogo.casa = classificados["1º Grupo D"];
//       jogo.fora = melhores[4]?.time || "";
//     }

//     if(jogo.id===82){
//       jogo.casa = classificados["1º Grupo G"];
//       jogo.fora = melhores[5]?.time || "";
//     }

//     if(jogo.id===83){
//       jogo.casa = classificados["2º Grupo K"];
//       jogo.fora = classificados["2º Grupo L"];
//     }

//     if(jogo.id===84){
//       jogo.casa = classificados["1º Grupo H"];
//       jogo.fora = classificados["2º Grupo J"];
//     }

//     if(jogo.id===85){
//       jogo.casa = classificados["1º Grupo B"];
//       jogo.fora = melhores[6]?.time || "";
//     }

//     if(jogo.id===86){
//       jogo.casa = classificados["1º Grupo J"];
//       jogo.fora = classificados["2º Grupo H"];
//     }

//     if(jogo.id===87){
//       jogo.casa = classificados["1º Grupo K"];
//       jogo.fora = melhores[7]?.time || "";
//     }

//     if(jogo.id===88){
//       jogo.casa = classificados["2º Grupo D"];
//       jogo.fora = classificados["2º Grupo G"];
//     }
//   });

//   localStorage.setItem(
//     "jogosSimulador",
//     JSON.stringify(jogosDetalhados)
//   );
// }

// function obterVencedor(jogo){
//   if(Number(jogo.placarCasa) > Number(jogo.placarFora))
//       return jogo.casa;

//   if(Number(jogo.placarFora) > Number(jogo.placarCasa))
//       return jogo.fora;

//   if(Number(jogo.penaisCasa) > Number(jogo.penaisFora))
//       return jogo.casa;

//   if(Number(jogo.penaisFora) > Number(jogo.penaisCasa))
//       return jogo.fora;
...
//   console.log("Preenchendo 16 avos...");
//   preencher16Avos();
// }
...
// /* ================= MONITORAMENTO E MAPEAMENTO DE ESTADO CENTRAL ================= */
// function init(){
//   const salvos = localStorage.getItem("jogosSimulador");
//   if (salvos) {
//     jogosDetalhados = JSON.parse(salvos);

//     // === PROTEÇÃO: Garante as novas propriedades nos jogos salvos sem apagar seus placares ===
//     jogosDetalhados.forEach(bloco => {
//       bloco.jogos.forEach(jogo => {
...
function criarAbas(){
  ...
  <button onclick="salvarPalpitesDefinitivos()" style="background-color: #28a745; color: white; margin-left: 15px; font-weight: bold; border: none; padding: 6px 12px; cursor: pointer; border-radius: 4px; display: inline-flex; align-items: center; justify-content: center; width: auto; min-width: max-content; white-space: nowrap;">💾 Salvar Palpite</button>`;
}
...
function atualizar(deveRenderizar = true){
  ...
  // 3. Mapeia quem são os classificados de cada grupo (Sem quebrar a estrutura da tabela)
  const classificadosPorGrupo = {};

  for(let g in tabela){
    let times = Object.entries(tabela[g]);
    // Ordenação oficial da tabela na tela: Pontos -> Saldo de Gols -> Posição Inicial
    times.sort((a,b)=> (b[1].pts - a[1].pts) || ((b[1].gp - b[1].gc) - (a[1].gp - a[1].gc)) || (a[1].pos - b[1].pos));
    
    // Pegamos os nomes ordenados para a validação das vagas do mata-mata
    const time1 = times[0] ? times[0][0] : "";
    const time2 = times[1] ? times[1][0] : "";
    const time3 = times[2] ? times[2][0] : "";

    // Validação das vagas: O país só vai pro mata-mata se ELE jogou 3 vezes
